    ...
    spawn: function(spawn) {
        ...
        if ( spawn.room.energyAvailable == spawn.room.energyCapacityAvailable ) {
            ...
            if ( false ) { //allExplorers.length == 0 && energyToUse >= 600){
                var insertMemory = { memory: { roleid: idnum, role: 'explorer', building: false, upgrading: false, storing: false }};
                var creepName = 'Explorer' + idnum;
                energyToUse -= 600;
                bodyWork += 1;
                opts.push(CLAIM);
            } else {
                var insertMemory = { memory: { roleid: idnum, role: 'worker', building: false, upgrading: false, storing: false }};
