...
var creepHarvester = {
    ...
    work: function (creep) {
        ...
        if (creep.carry.energy < creep.carryCapacity && !creep.memory.building && !creep.memory.upgrading && !creep.memory.storing) {
            ...
            /*if (targetStructures.length > 0 && creep.room.controller.ticksToDowngrade >= 1000) {
                ...
                if (randomEnergyStorage >= targetStructures.length || randomEnergyStorage == 0) {
                    randomEnergyStorage = Math.round(Math.random(0, targetStructures.length) * targetStructures.length);
                }
                console.log(randomEnergyStorage, targetStructures.length );
                //if 
